...
typedef struct {
  ...
  real_T ADMIT_RATIO;                  // '<S1>/ADMIT_RATIO'
  real_T P_prev[36];                   // '<S1>/P_prev'
  real_T R;                            // '<S1>/R'
  real_T acc_xy;                       // '<S1>/acc_xy'
  real_T acc_z;                        // '<S1>/acc_z'
  real_T x_prev[6];                    // '<S1>/x_prev'
} DW_ekf_iod_T;
...
struct P_ekf_iod_T_ {
  ...
  real_T ekf_iod_x_hat0[6];            // Mask Parameter: ekf_iod_x_hat0
                                       //  Referenced by:
                                       //    '<S1>/x_prev'
                                       //    '<S1>/Unit Delay1'

  real_T ADMIT_RATIO_InitialValue;     // Expression: 1.5
...
class ekfIodClass {
 ...
 public:
  ...
  // model step function
  void step(real_T arg_dists[4], real_T arg_deltat, real_T arg_imu, uint8_T
            arg_nodeId, boolean_T arg_calibEnable, real_T arg_ancs[12], real_T
...
//  '<S3>'   : 'ekf_iod/ekf_iod/ekf prediction'
//  '<S4>'   : 'ekf_iod/ekf_iod/outlier detection & ekf update'
