...
void usage(void)
{
    ...
    printf("\nTo connect to radio at IP address 192.168.1.300 via Ethernet:\n");
    printf("\trcmSampleApp -i 192.168.1.300\n");

#ifdef WIN32
    printf("\nTo connect to radio's serial port at COM3:\n");
    printf("\trcmSampleApp -s COM4\n");
...
int main(int argc, char *argv[])
{
    ...
    if (rcmIfInit(rcmIf, "/dev/ttyUSB0") != OK)
    {
    ...
    while(ros::ok())
    {
        destNodeId = DEFAULT_DEST_NODE_ID;
